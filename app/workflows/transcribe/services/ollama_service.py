...
class OllamaChunker:
    ...
    async def chunk_by_topics(
        ...
        For long transcripts, uses a sliding window approach to analyze
        manageable sections and then combines the results.
        ...
        try:
            ...
            # For very long transcripts, use sliding window approach
            if estimated_tokens > 8000:  # Model context limit
                logger.info(f"Long transcript ({estimated_tokens} tokens), using sliding window analysis")
                return await self._chunk_long_transcript(transcript, segments, progress)
...
"""
    ...
    def _fallback_sentence_chunking(
        ...
        for sentence in sentences:
            ...
            if current_length + sentence_length > target_size and current_chunk:
                ...
                first_sentence = chunk_text.split('.')[0].split('!')[0].split('?')[0].strip()
                if len(first_sentence) > 100:
                    first_sentence = first_sentence[:97] + "..."
                topic_summary = first_sentence if first_sentence else "Transcript segment"
    ...
    async def _chunk_long_transcript(
        self,
        transcript: str,
        segments: List[Dict[str, Any]],
        progress: Optional[ProgressTracker] = None
        ...
        """
        Handle long transcripts by using a sliding window approach.

        For now, fall back to sentence chunking since windowed analysis
        is complex and the current transcript isn't actually that long.
        """
        logger.warning("Long transcript detected but windowed analysis not fully implemented. Using sentence chunking fallback.")
        return self._fallback_sentence_chunking(transcript, segments)
